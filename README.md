...
Use openssl to convert certificate in _.pem_ (pkcs12) format from _.pfx_ and password provided by bank:
...
const status = async function () {
  const result = await maib
    .createCommand()
    .getTransactionStatus('TRANSACTION_ID_HERE');
...
const closeday = async function () {
  const result = await maib.createCommand().closeDay();
...
const deleterec = async function () {
  const result = await maib.createCommand().deleteRegularPayment('CARD_ID');
